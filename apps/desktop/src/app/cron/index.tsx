...
export function CronView({ setStatusbarItemGroup: _setStatusbarItemGroup, ...props }: CronViewProps) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      notifyError(err, zh ? '加载定时任务失败' : 'Failed to load cron jobs')
    } finally {
      setRefreshing(false)
    }
  }, [])
  ...
  async function handlePauseResume(job: CronJob) {
    ...
    try {
      ...
      notify({
        kind: 'success',
        title: isPaused ? (zh ? '定时任务已恢复' : 'Cron resumed') : zh ? '定时任务已暂停' : 'Cron paused',
        message: truncate(jobTitle(job), 60)
      })
    } catch (err) {
      notifyError(err, zh ? '更新定时任务失败' : 'Failed to update cron job')
    } finally {
      setBusyJobId(null)
    }
  }
  ...
  async function handleTrigger(job: CronJob) {
    ...
    try {
      ...
      setJobs(current => (current ? current.map(row => (row.id === job.id ? updated : row)) : current))
      notify({ kind: 'success', title: zh ? '定时任务已触发' : 'Cron triggered', message: truncate(jobTitle(job), 60) })
    } catch (err) {
      notifyError(err, zh ? '触发定时任务失败' : 'Failed to trigger cron job')
    } finally {
      setBusyJobId(null)
    }
  }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      notify({ kind: 'success', title: zh ? '定时任务已删除' : 'Cron deleted', message: truncate(jobTitle(pendingDelete), 60) })
      setPendingDelete(null)
    } catch (err) {
      notifyError(err, zh ? '删除定时任务失败' : 'Failed to delete cron job')
    } finally {
      setDeleting(false)
    }
  }
  ...
  async function handleEditorSave(values: EditorValues) {
    if (editor.mode === 'create') {
      ...
      notify({ kind: 'success', title: zh ? '定时任务已创建' : 'Cron created', message: truncate(jobTitle(created), 60) })
      ...
      notify({ kind: 'success', title: zh ? '定时任务已更新' : 'Cron updated', message: truncate(jobTitle(updated), 60) })
    }
  ...
  return (
    <PageSearchShell
      ...
      searchPlaceholder={zh ? '搜索定时任务…' : 'Search cron jobs...'}
      searchTrailingAction={
        <Button
          aria-label={refreshing ? (zh ? '正在刷新定时任务' : 'Refreshing cron jobs') : zh ? '刷新定时任务' : 'Refresh cron jobs'}
          className="text-(--ui-text-tertiary) hover:bg-(--chrome-action-hover) hover:text-foreground"
          disabled={refreshing}
          onClick={() => void refresh()}
          size="icon-xs"
          title={refreshing ? (zh ? '正在刷新定时任务' : 'Refreshing cron jobs') : zh ? '刷新定时任务' : 'Refresh cron jobs'}
          type="button"
          variant="ghost"
        >
    ...
    >
      {!jobs ? (
        <PageLoader label={zh ? '正在加载定时任务…' : 'Loading cron jobs...'} />
        ...
        <EmptyState
          actionLabel={totalCount === 0 ? (zh ? '创建第一个定时任务' : 'Create first cron') : undefined}
          description={
            totalCount === 0
              ? (zh ? '给一个提示词设定运行时间。Hermes 会按计划执行，并把结果发到你选的目标位置。' : 'Schedule a prompt to run on a cron expression. Hermes will run it and deliver results to the destination you pick.')
              : zh ? '换个更宽一点的搜索词试试。' : 'Try a broader search query.'
          }
          onAction={totalCount === 0 ? () => setEditor({ mode: 'create' }) : undefined}
          title={totalCount === 0 ? (zh ? '还没有定时任务' : 'No scheduled jobs yet') : zh ? '没有匹配结果' : 'No matches'}
        />
      ) : (
        <div className="h-full overflow-y-auto px-4 py-3">
          ...
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[0.7rem] uppercase tracking-wide text-muted-foreground">
              {zh ? `${enabledCount}/${totalCount} 个已启用` : `${enabledCount}/${totalCount} active`}
            </span>
      ...
      <Dialog onOpenChange={open => !open && !deleting && setPendingDelete(null)} open={pendingDelete !== null}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{zh ? '删除这个定时任务？' : 'Delete cron job?'}</DialogTitle>
            <DialogDescription>
              {pendingDelete ? (
                <>
                  {zh ? '这会永久删除 ' : 'This will remove '}
                  <span className="font-medium text-foreground">{truncate(jobTitle(pendingDelete), 60)}</span>{' '}
                  {zh ? '，而且会立刻停止执行。' : 'permanently. It will stop firing immediately.'}
                </>
              ) : null}
            </DialogDescription>
          </DialogHeader>
    ...
    <div className="grid gap-3 px-3 py-2.5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
      ...
      >
        ...
        <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-[0.68rem] text-muted-foreground">
          ...
          <span>{zh ? '上次：' : 'Last: '}{formatTime(job.last_run_at)}</span>
          <span>{zh ? '下次：' : 'Next: '}{formatTime(job.next_run_at)}</span>
        </div>
        {job.last_error && (
      ...
      <div className="flex shrink-0 items-center gap-0.5">
        <IconAction
          aria-label={isPaused ? (zh ? '恢复定时任务' : 'Resume cron') : zh ? '暂停定时任务' : 'Pause cron'}
          disabled={busy}
          onClick={onPauseResume}
          title={isPaused ? (zh ? '恢复' : 'Resume') : zh ? '暂停' : 'Pause'}
        >
          {isPaused ? <Play className="size-3.5" /> : <Pause className="size-3.5" />}
        </IconAction>
        <IconAction aria-label={zh ? '立即触发' : 'Trigger now'} disabled={busy} onClick={onTrigger} title={zh ? '立即触发' : 'Trigger now'}>
          <Zap className="size-3.5" />
        </IconAction>
        <IconAction aria-label={zh ? '编辑定时任务' : 'Edit cron'} onClick={onEdit} title={zh ? '编辑' : 'Edit'}>
          <Pencil className="size-3.5" />
        </IconAction>
        <IconAction
          aria-label={zh ? '删除定时任务' : 'Delete cron'}
...
function CronEditorDialog({
  ...
  const deliveryOptions = DELIVERY_OPTIONS.map(option => ({
    ...option,
    label:
      !zh
        ? option.label
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (!trimmedPrompt || !trimmedSchedule) {
      setError(zh ? '提示词和执行时间都是必填项。' : 'Prompt and schedule are required.')
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : zh ? '保存定时任务失败' : 'Failed to save cron job')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Dialog onOpenChange={value => !value && !saving && onClose()} open={open}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{isEdit ? (zh ? '编辑定时任务' : 'Edit cron job') : zh ? '新建定时任务' : 'New cron job'}</DialogTitle>
          <DialogDescription>
            {isEdit
              ? (zh ? '你可以修改执行时间、提示词或投递目标，改动会在下一次运行时生效。' : 'Update the schedule, prompt, or delivery target. Changes apply on next run.')
              : zh ? '给提示词安排自动运行时间。可以用 cron 表达式，也可以写像“每 15 分钟一次”这样的自然语句。' : 'Schedule a prompt to run automatically. Use cron syntax or a natural phrase like "every 15 minutes".'}
          </DialogDescription>
        </DialogHeader>

        <form className="grid gap-4" onSubmit={handleSubmit}>
          <Field htmlFor="cron-name" label={zh ? '名称' : 'Name'} optional>
            <Input
              autoFocus
              id="cron-name"
              onChange={event => setName(event.target.value)}
              placeholder={zh ? '例如：早报汇总' : 'Morning briefing'}
              value={name}
            />
          </Field>

          <Field htmlFor="cron-prompt" label={zh ? '提示词' : 'Prompt'}>
            <Textarea
              className="min-h-24 font-mono"
              id="cron-prompt"
              onChange={event => setPrompt(event.target.value)}
              placeholder={zh ? '例如：总结我还没读的 Slack 线程，把最重要的 5 条发邮件给我…' : 'Summarize my unread Slack threads and email me the top 5...'}
              value={prompt}
            />
          </Field>

          <div className="grid items-start gap-4 sm:grid-cols-2">
            <Field htmlFor="cron-frequency" label={zh ? '频率' : 'Frequency'}>
            ...
            <Field htmlFor="cron-deliver" label={zh ? '发送到' : 'Deliver to'}>
          ...
          {schedulePreset === 'custom' ? (
            <Field htmlFor="cron-schedule" label={zh ? '自定义时间' : 'Custom schedule'}>
              <Input
                className="font-mono"
                id="cron-schedule"
                onChange={event => setSchedule(event.target.value)}
                placeholder={zh ? '例如：0 9 * * * 或 weekdays at 9am' : '0 9 * * * or weekdays at 9am'}
                value={schedule}
              />
              <FieldHint>{zh ? '可以填 cron 表达式，也可以写“every hour”“weekdays at 9am”这类自然语句。' : 'Cron expression, or phrases like "every hour" or "weekdays at 9am".'}</FieldHint>
            </Field>
          ) : (
          ...
          <DialogFooter>
            ...
            <Button disabled={saving} type="submit">
              {saving ? (zh ? '保存中…' : 'Saving...') : isEdit ? (zh ? '保存更改' : 'Save changes') : zh ? '创建定时任务' : 'Create cron'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
