...
export function TitlebarControls({
  ...
  const leftToolbarTools: TitlebarTool[] = [
    {
      ...
      id: 'sidebar',
      label: sidebarOpen ? (zh ? '隐藏侧边栏' : 'Hide sidebar') : zh ? '显示侧边栏' : 'Show sidebar',
      onSelect: () => {
        triggerHaptic('tap')
        toggleSidebarOpen()
      }
    },
    {
      ...
      title: zh ? '搜索会话、页面和操作' : 'Search sessions, views, and actions'
    },
    ...leftTools
  ]
  ...
  const rightSidebarTool: TitlebarTool = {
    ...
    id: 'right-sidebar',
      label: fileBrowserOpen ? (zh ? '隐藏右侧栏' : 'Hide right sidebar') : zh ? '显示右侧栏' : 'Show right sidebar',
    onSelect: () => {
      triggerHaptic('tap')
  ...
  const systemTools: TitlebarTool[] = [
    {
      ...
      id: 'haptics',
      label: hapticsMuted ? (zh ? '打开触感反馈' : 'Unmute haptics') : zh ? '关闭触感反馈' : 'Mute haptics',
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-64" sideOffset={8}>
        <DropdownMenuLabel>
          <div className="text-sm font-medium text-foreground">{zh ? '配置档案' : 'Profiles'}</div>
          <div className="mt-1 text-xs font-normal leading-4 text-muted-foreground">
            {zh ? '给不同人格、配置、技能和 SOUL.md 分开一套 Hermes 环境。' : 'Advanced Hermes environments for separate personas, config, skills, and SOUL.md.'}
          </div>
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
        <DropdownMenuItem
        ...
        >
          ...
          <span>{zh ? '管理配置档案' : 'Manage profiles'}</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
