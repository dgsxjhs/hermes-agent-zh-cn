...
function EnvActions({
  ...
  return (
    <div className="flex shrink-0 items-center gap-1.5">
      {info.url && (
        <Button asChild size="xs" title={zh ? '打开提供商文档' : 'Open provider docs'} variant="ghost">
      ...
      {info.is_set && showReveal && (
        <Button
          ...
          size="icon-xs"
          title={isRevealed ? (zh ? '隐藏值' : 'Hide value') : zh ? '显示值' : 'Reveal value'}
          variant="ghost"
        >
      ...
      <Button onClick={onEdit} size="xs" variant="outline">
        {info.is_set ? (zh ? '替换' : 'Replace') : zh ? '设置' : 'Set'}
      </Button>
      {info.is_set && (
        <Button
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Pill tone={info.is_set ? 'primary' : 'muted'}>
              ...
              {info.is_set ? (zh ? '已设置' : 'Set') : zh ? '未设置' : 'Not set'}
            </Pill>
          </div>
      ...
      {isEditing && (
        <div className="flex flex-wrap items-center gap-2">
          <Input
            ...
            placeholder={info.is_set ? (zh ? '替换当前值' : 'Replace current value') : zh ? '输入值' : 'Enter value'}
            type={info.is_password ? 'password' : 'text'}
            value={edits[varKey]}
          />
          <Button disabled={saving === varKey || !edits[varKey]} onClick={() => onSave(varKey)} size="sm">
            <Save />
            {saving === varKey ? (zh ? '保存中' : 'Saving') : zh ? '保存' : 'Save'}
          </Button>
      ...
      >
        <span className="flex min-w-0 items-center gap-2">
          ...
          <span className="truncate text-sm font-medium">
            {group.name === 'Other' ? (zh ? '其他提供商' : 'Other providers') : group.name}
          </span>
          {setCount > 0 && <Pill tone="primary">{zh ? `${setCount} 项已设` : `${setCount} set`}</Pill>}
        </span>
        <span className="text-xs text-muted-foreground">{zh ? `${group.entries.length} 个密钥` : `${group.entries.length} keys`}</span>
      </button>
      {expanded && (
...
export function KeysSettings({ query }: SearchProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        notifyError(err, zh ? 'API 密钥加载失败' : 'API keys failed to load')
      }
    })()
  ...
  const otherGroups = useMemo(() => {
    ...
    const labels: Record<string, string> = {
      tool: zh ? '工具' : 'Tools',
      messaging: zh ? '消息平台' : 'Messaging',
      setting: zh ? '设置' : 'Settings'
    }
  ...
  async function handleSave(key: string) {
    ...
    try {
      ...
      clearLocalState(key)
      notify({ kind: 'success', title: zh ? '凭据已保存' : 'Credential saved', message: zh ? `${key} 已更新。` : `${key} updated.` })
    } catch (err) {
      notifyError(err, zh ? `保存 ${key} 失败` : `Failed to save ${key}`)
    } finally {
      setSaving(null)
    }
  }
  ...
  async function handleClear(key: string) {
    if (!window.confirm(zh ? `要从 .env 里移除 ${key} 吗？` : `Remove ${key} from .env?`)) {
      return
    }
    ...
    try {
      ...
      clearLocalState(key)
      notify({ kind: 'success', title: zh ? '凭据已移除' : 'Credential removed', message: zh ? `${key} 已移除。` : `${key} removed.` })
    } catch (err) {
      notifyError(err, zh ? `移除 ${key} 失败` : `Failed to remove ${key}`)
    } finally {
      setSaving(null)
    }
  }
  ...
  async function handleReveal(key: string) {
    ...
    try {
      ...
      notifyError(err, zh ? `显示 ${key} 失败` : `Failed to reveal ${key}`)
    }
  }

  if (!vars) {
    return <LoadingState label={zh ? '正在加载 API 密钥和凭据…' : 'Loading API keys and credentials...'} />
  }
  ...
  return (
    <SettingsContent>
      <div className="mb-6">
        <SectionHeading
          icon={Zap}
          meta={zh ? `已配置 ${configuredCount} / ${providerGroups.length}` : `${configuredCount} of ${providerGroups.length} configured`}
          title={zh ? '大模型提供商' : 'LLM providers'}
        />
      ...
      {otherGroups.map(group => (
        <div className="mb-6" key={group.category}>
          <SectionHeading
            icon={Settings2}
            meta={zh ? `已设置 ${group.entries.filter(([, i]) => i.is_set).length} / ${group.entries.length}` : `${group.entries.filter(([, i]) => i.is_set).length} of ${group.entries.length} set`}
            title={group.label}
          />
          <div className="grid gap-2">
            {group.entries.map(([key, info]) => (
