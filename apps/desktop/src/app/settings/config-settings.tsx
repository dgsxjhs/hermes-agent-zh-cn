...
function ConfigField({
  ...
  const { t } = useLocale()
  const translationKey = schemaKey.replace(/\./g, '_')
  const fallbackLabel = FIELD_LABELS[schemaKey] ?? prettyName(schemaKey.split('.').pop() ?? schemaKey)
  const translatedLabel = t(`settingsFields.${translationKey}`)
  const label = translatedLabel === `settingsFields.${translationKey}` ? fallbackLabel : translatedLabel
  const normalize = (v: string) => v.toLowerCase().replace(/[^a-z0-9]+/g, '')
  const fallbackDescription = (FIELD_DESCRIPTIONS[schemaKey] ?? schema.description ?? '').trim()
  const translatedDescription = t(`settingsFieldDescriptions.${translationKey}`)
  const rawDescription =
    translatedDescription === `settingsFieldDescriptions.${translationKey}` ? fallbackDescription : translatedDescription
  ...
  if (schema.type === 'boolean') {
    return row(
      <div className="flex items-center justify-end gap-3">
        <span className="text-xs text-muted-foreground">{value ? t('settings.booleanOn') : t('settings.booleanOff')}</span>
            ...
            <SelectItem key={option || EMPTY_SELECT_VALUE} value={option || EMPTY_SELECT_VALUE}>
              {option
                ...
                : schemaKey === 'display.personality'
                  ? t('settings.nonePersonality')
                  : t('settings.noneGeneric')}
            </SelectItem>
          ))}
        </SelectContent>
      </Select>
...
export function ConfigSettings({
  ...
  useEffect(() => {
    ...
    Promise.all([getHermesConfigRecord(), getHermesConfigDefaults(), getHermesConfigSchema()])
      ...
      .catch(err => notifyError(err, t('settings.loadFailed')))
    ...
    const timeoutId = window.setTimeout(() => {
      void (async () => {
        try {
          ...
          if (saveVersionRef.current === v) {
            notifyError(err, t('settings.autosaveFailed'))
          }
        }
      })()
    }, 550)
  ...
  function handleImport(e: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        ...
        notify({ kind: 'success', title: t('settings.importedTitle'), message: t('settings.importedMessage') })
      } catch (err) {
        notifyError(err, t('settings.invalidJson'))
      }
    }
  ...
  if (!config || !schema) {
    return <LoadingState label={t('settings.loadingConfig')} />
  }
    ...
    <SettingsContent>
      ...
      {query.trim() && (
        <div className="mb-4 text-xs text-muted-foreground">
          {t('settings.resultsCount', { count: fields.length, suffix: fields.length === 1 ? '' : 's' })}
        </div>
      )}
      {fields.length === 0 ? (
        <EmptyState description={t('settings.noResultsBody')} title={t('settings.noResultsTitle')} />
      ) : (
