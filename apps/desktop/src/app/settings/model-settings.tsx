...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  if (loading && !mainModel) {
    return <LoadingState label={zh ? '正在加载模型配置…' : 'Loading model configuration...'} />
  }
    ...
    <div className="grid gap-6">
      <section>
        <SectionHeading
          ...
          title={zh ? '主模型' : 'Main model'}
        />
        <p className="mb-3 text-xs text-muted-foreground">
          {zh ? '只影响新会话。当前聊天如果想临时切模型，请用输入框那里的模型选择器。' : 'Applies to new sessions. Use the model picker in the composer to hot-swap the active chat.'}
        </p>
        <div className="flex flex-wrap items-center gap-2">
          <Select onValueChange={setSelectedProvider} value={selectedProvider}>
            <SelectTrigger className={cn('min-w-40', CONTROL_TEXT)}>
              <SelectValue placeholder={zh ? '提供方' : 'Provider'} />
            </SelectTrigger>
            <SelectContent>
          ...
          <Select onValueChange={setSelectedModel} value={selectedModel}>
            <SelectTrigger className={cn('min-w-60', CONTROL_TEXT)}>
              <SelectValue placeholder={zh ? '模型' : 'Model'} />
            </SelectTrigger>
            <SelectContent>
      ...
      <section>
        <div className="mb-2.5 flex items-center justify-between">
          <SectionHeading icon={Cpu} title={zh ? '辅助模型' : 'Auxiliary models'} />
          <Button
            ...
            variant="outline"
          >
            {zh ? '全部重置为主模型' : 'Reset all to main'}
          </Button>
        </div>
        <p className="mb-2 text-xs text-muted-foreground">
          {zh ? '这些辅助任务默认跟随主模型。你也可以单独给某个任务指定专用模型。' : 'Helper tasks run on the main model by default. Assign a dedicated model to any task to override.'}
        </p>
              ...
              <ListRow
                ...
                description={
                  <span className="font-mono text-[0.68rem]">
                    {isAuto ? (zh ? '自动 · 使用主模型' : 'auto · use main model') : `${current.provider} · ${current.model || (zh ? '（提供方默认）' : '(provider default)')}`}
                  </span>
                }
                key={meta.key}
                title={
