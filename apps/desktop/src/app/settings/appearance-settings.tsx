...
export function AppearanceSettings() {
  ...
  const { t } = useLocale()
  const desktopLocale = useStore($desktopLocale)
  const toolViewMode = useStore($toolViewMode)
  const activeTheme = availableThemes.find(t => t.name === themeName)
  const modeOptions = [
    { id: 'light', label: t('appearance.modeLight'), description: t('appearance.modeLightDescription'), icon: Sun },
    { id: 'dark', label: t('appearance.modeDark'), description: t('appearance.modeDarkDescription'), icon: Moon },
    { id: 'system', label: t('appearance.modeSystem'), description: t('appearance.modeSystemDescription'), icon: Monitor }
  ] as const
    ...
    <SettingsContent>
      <div className="space-y-5">
        ...
        <section className="rounded-xl border border-(--ui-stroke-tertiary) bg-(--ui-chat-bubble-background) p-3 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-muted-foreground">{t('appearance.languageDescription')}</div>
            </div>
            <Pill>{desktopLocale === 'zh-CN' ? t('appearance.languageChinese') : t('appearance.languageEnglish')}</Pill>
          </div>
          <div className="grid gap-2 sm:grid-cols-2">
            {([
              { id: 'en', label: t('appearance.languageEnglish') },
              { id: 'zh-CN', label: t('appearance.languageChinese') }
            ] as const).map(option => {
              const active = desktopLocale === option.id
        ...
        <section className="rounded-xl border border-(--ui-stroke-tertiary) bg-(--ui-chat-bubble-background) p-3 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-medium">{t('appearance.colorMode')}</div>
              <div className="mt-1 text-xs text-muted-foreground">{t('appearance.colorModeDescription')}</div>
            </div>
        ...
        <section className="rounded-xl border border-(--ui-stroke-tertiary) bg-(--ui-chat-bubble-background) p-3 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-medium">{t('appearance.toolDisplay')}</div>
              <div className="mt-1 text-xs text-muted-foreground">{t('appearance.toolDisplayDescription')}</div>
            </div>
            <Pill>{toolViewMode === 'technical' ? t('appearance.toolDisplayTechnical') : t('appearance.toolDisplayProduct')}</Pill>
          </div>
          <div className="grid gap-2 sm:grid-cols-2">
            {(
              [
                {
                  id: 'product',
                  label: t('appearance.toolDisplayProduct'),
                  description: t('appearance.toolDisplayProductDescription')
                },
                {
                  id: 'technical',
                  label: t('appearance.toolDisplayTechnical'),
                  description: t('appearance.toolDisplayTechnicalDescription')
                }
              ] as const
            ).map(option => {
        ...
        <section className="rounded-xl border border-(--ui-stroke-tertiary) bg-(--ui-chat-bubble-background) p-3 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-muted-foreground">{t('appearance.themeDescription')}</div>
            </div>
