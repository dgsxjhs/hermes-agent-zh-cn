...
const stateLabel = (state?: null | string, zh = false) =>
  state
    ? zh
      ? {
          connected: '已连接',
...
export function MessagingView({ setStatusbarItemGroup: _setStatusbarItemGroup, ...props }: MessagingViewProps) {
  ...
  const refreshPlatforms = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!silent) {
        notifyError(err, zh ? '消息平台加载失败' : 'Messaging platforms failed to load')
      }
    } finally {
      if (!silent) {
        setRefreshing(false)
  ...
  async function handleToggle(platform: MessagingPlatformInfo, enabled: boolean) {
    ...
    try {
      ...
      notify({
        kind: 'success',
        title: enabled ? `${platform.name}${zh ? ' 已启用' : ' enabled'}` : `${platform.name}${zh ? ' 已停用' : ' disabled'}`,
        message: zh ? '重启网关后，这个改动才会生效。' : 'Restart the gateway for this change to take effect.'
      })
    } catch (err) {
      notifyError(err, zh ? `更新 ${platform.name} 失败` : `Failed to update ${platform.name}`)
    } finally {
      setSaving(null)
    }
  }
  ...
  async function handleSave(platform: MessagingPlatformInfo) {
    ...
    try {
      ...
      notify({
        kind: 'success',
        title: zh ? `${platform.name} 配置已保存` : `${platform.name} setup saved`,
        message: zh ? '重启网关后，会用新凭据重新连接。' : 'Restart the gateway to reconnect with the new credentials.'
      })
    } catch (err) {
      notifyError(err, zh ? `保存 ${platform.name} 失败` : `Failed to save ${platform.name}`)
    } finally {
      setSaving(null)
    }
  }
  ...
  async function handleClear(platform: MessagingPlatformInfo, key: string) {
    ...
    try {
      ...
      await refreshPlatforms()
      notify({ kind: 'success', title: zh ? `${key} 已清空` : `${key} cleared`, message: zh ? `${platform.name} 配置已更新。` : `${platform.name} setup was updated.` })
    } catch (err) {
      notifyError(err, zh ? `清空 ${key} 失败` : `Failed to clear ${key}`)
    } finally {
      setSaving(null)
    }
  }
  ...
  return (
    <PageSearchShell
      ...
      searchPlaceholder={zh ? '搜索消息平台…' : 'Search messaging...'}
      searchTrailingAction={null}
      searchValue={query}
    >
      {!platforms ? (
        <PageLoader label={zh ? '正在加载消息平台…' : 'Loading messaging platforms...'} />
      ) : (
    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="min-h-0 flex-1 overflow-y-auto">
        <div className="mx-auto max-w-2xl space-y-5 px-5 py-4">
          <header className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <StatePill tone={stateTone(platform)}>{stateLabel(platform.state, zh)}</StatePill>
                <SetupPill active={platform.configured}>
                  {platform.configured ? (zh ? '已配置凭据' : 'Credentials set') : zh ? '需要配置' : 'Needs setup'}
                </SetupPill>
                {!platform.gateway_running && <SetupPill active={false}>{zh ? '消息网关已停止' : 'Messaging gateway stopped'}</SetupPill>}
              </div>
          ...
          <section>
            <SectionTitle>{zh ? '获取凭据' : 'Get your credentials'}</SectionTitle>
            <p className="mt-1 text-[length:var(--conversation-caption-font-size)] leading-(--conversation-caption-line-height) text-(--ui-text-tertiary)">
              {introCopy(platform, zh)}
            </p>
            <div className="mt-3">
              <Button asChild size="sm" variant="outline">
                <a href={platform.docs_url} rel="noreferrer" target="_blank">
                  {zh ? '打开配置指引' : 'Open setup guide'}
          ...
          <section>
            <SectionTitle>{zh ? '必填' : 'Required'}</SectionTitle>
            <div className="mt-3 space-y-4">
              {requiredFields.length > 0 ? (
                ...
                <p className="text-[length:var(--conversation-caption-font-size)] leading-(--conversation-caption-line-height) text-(--ui-text-tertiary)">
                  {zh ? '这个平台这里不需要单独填 token。先按上面的配置指引操作，再在下面启用它。' : 'This platform does not need a token here. Use the setup guide above, then enable it below.'}
                </p>
              )}
            </div>
          </section>
          ...
          {optionalFields.length > 0 && (
            <section>
              <SectionTitle>{zh ? '推荐填写' : 'Recommended'}</SectionTitle>
              <div className="mt-3 space-y-4">
          ...
          {hiddenCount > 0 && (
            <section>
              <button
                ...
                type="button"
              >
                <span>{zh ? `高级选项（${hiddenCount}）` : `Advanced (${hiddenCount})`}</span>
        ...
        <div className="mx-auto flex max-w-2xl flex-wrap items-center gap-2">
          <label className="flex shrink-0 items-center gap-2 rounded-md border border-(--ui-stroke-tertiary) bg-(--ui-bg-quinary) px-2.5 py-1.5 text-[length:var(--conversation-text-font-size)]">
            ...
            <span className="text-xs font-medium text-muted-foreground">
              {platform.enabled ? (zh ? '已启用' : 'Enabled') : zh ? '已停用' : 'Disabled'}
            </span>
          </label>

          <div className="ml-auto flex items-center gap-2">
            {hasEdits && <span className="text-xs text-muted-foreground">{zh ? '有未保存的改动' : 'Unsaved changes'}</span>}
            <Button disabled={!hasEdits || isSavingEnv} onClick={onSave} size="sm">
              <Save />
              {isSavingEnv ? (zh ? '保存中…' : 'Saving...') : zh ? '保存更改' : 'Save changes'}
            </Button>
          </div>
        </div>
      </footer>
...
const PLATFORM_INTRO_EN: Record<string, string> = {
  telegram:
...
const PLATFORM_INTRO_ZH: Record<string, string> = {
  telegram: '在 Telegram 里先找 @BotFather，执行 /newbot 拿到 token，再去 @userinfobot 获取你的数字用户 ID。',
  discord: '打开 Discord Developer Portal，新建应用并添加 Bot，然后复制它的 token，再把机器人按正确权限邀请进你的服务器。',
  slack: '创建 Slack 应用并启用 Socket Mode，安装到你的工作区后，复制 Bot token（xoxb-）和 App 级 token（xapp-）。',
  mattermost: '在你的 Mattermost 服务器里创建一个机器人账号或个人访问令牌，然后把服务器地址和 token 填到这里。',
  matrix: '用机器人账号登录你的 homeserver，然后复制 access token、用户 ID 和 homeserver 地址。',
  signal: '先运行一个可访问的 signal-cli REST bridge，再把它的地址和已注册手机号填给 Hermes。',
  whatsapp: '启动 Hermes 自带的 WhatsApp bridge，首次运行时扫一次二维码，然后再启用这个平台。',
  bluebubbles: '在带 iMessage 的 Mac 上运行 BlueBubbles Server，暴露它的 API，然后把地址和服务器密码填到这里。',
  homeassistant: '在 Home Assistant 里打开个人资料，创建一个长期访问令牌，然后连同 HA 地址一起填到这里。',
  email: '建议使用专门邮箱。若是 Gmail 或 Workspace，先创建应用专用密码，再使用 imap.gmail.com 和 smtp.gmail.com。',
  sms: '去 Twilio 控制台拿到 Account SID 和 Auth Token，再准备一个能发短信的号码。',
  dingtalk: '在钉钉开发者后台创建应用，然后把 Client ID（App Key）和 Client Secret 填到这里。',
  feishu: '创建飞书 / Lark 应用，配置机器人能力后，把 App ID、App secret 和事件加密密钥填进来。',
  wecom: '在企业微信里添加群机器人，把 webhook key 作为 WECOM_BOT_ID 填入。这里只能发送；双向收发请用 WeCom（app）。',
  wecom_callback: '配置企业微信自建应用，暴露回调 URL，并提供 corp ID、secret、agent ID 和 AES key。',
  weixin: '登录微信公众号平台，复制 AppID 和 Token，并把消息回调地址指向 Hermes。',
  qqbot: '去 QQ 开放平台（q.qq.com）注册应用，然后复制 App ID 和 Client Secret。',
  api_server: '把 Hermes 暴露成兼容 OpenAI 的 API。设置好认证密钥后，再让 Open WebUI、LobeChat 等指向这个 host:port。',
  webhook: '运行一个 HTTP 服务，接收 GitHub、GitLab 或自定义应用的 POST 请求，并用 secret 校验签名。'
}

const introCopy = (platform: MessagingPlatformInfo, zh = false) =>
  (zh ? PLATFORM_INTRO_ZH[platform.id] : PLATFORM_INTRO_EN[platform.id]) || platform.description
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex flex-wrap items-baseline gap-2">
        ...
        {field.is_set && <span className="text-[0.66rem] font-medium text-primary">{zh ? '已保存' : 'Saved'}</span>}
      </div>
      <div className="flex items-center gap-2">
        <Input
          ...
          placeholder={field.is_set ? field.redacted_value || (zh ? '替换当前值' : 'Replace current value') : copy.placeholder}
          type={field.is_password ? 'password' : 'text'}
          value={edits[field.key] || ''}
        />
        {field.url && (
          <Button asChild size="icon-sm" title={zh ? '打开文档' : 'Open docs'} variant="ghost">
        ...
        {field.is_set && (
          <Button
            ...
            size="icon-sm"
            title={zh ? `清空 ${field.key}` : `Clear ${field.key}`}
            variant="ghost"
          >
