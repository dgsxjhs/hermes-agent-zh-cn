...
function statusGlyph(status: SubagentStatus, zh: boolean): ReactNode {
  if (status === 'running' || status === 'queued') {
    return (
      <BrailleSpinner
        ariaLabel={zh ? '运行中' : 'Running'}
  ...
  if (status === 'failed' || status === 'interrupted') {
    return <AlertCircle aria-label={zh ? '失败' : 'Failed'} className="size-3.5 shrink-0 text-destructive" />
  }

  return <CheckCircle2 aria-label={zh ? '已完成' : 'Done'} className="size-3.5 shrink-0 text-emerald-600/85 dark:text-emerald-400/85" />
}
...
export function AgentsView({ onClose }: AgentsViewProps) {
  ...
  return (
    <OverlayView
      closeLabel={zh ? '关闭子代理面板' : 'Close agents'}
      contentClassName="px-5 pt-5 pb-4 sm:px-6"
      onClose={onClose}
      rootClassName="mx-auto max-w-3xl"
    >
      <header className="mb-3 shrink-0">
        <h2 className="text-sm font-semibold text-foreground">{zh ? '任务分叉树' : 'Spawn tree'}</h2>
        <p className="text-xs text-muted-foreground/80">{zh ? '这里会显示当前轮次里子代理的实时活动。' : 'Live subagent activity for the current turn.'}</p>
      </header>
...
const fmtAge = (updatedAt: number, nowMs: number, zh = false) => {
...
function SubagentTree({ tree }: { tree: SubagentNode[] }) {
  ...
  if (tree.length === 0) {
    return (
      <div className="grid place-items-center gap-3 py-12 text-center">
        ...
        <p className="text-sm font-medium text-foreground/90">{zh ? '当前没有活跃的子代理' : 'No live subagents'}</p>
        <p className="max-w-md text-xs leading-relaxed text-muted-foreground/75">
          {zh ? '当这一轮把任务分给子代理时，它们的进度会实时显示在这里。' : 'When a turn delegates work, child agents stream their progress here.'}
        </p>
      </div>
    )
  }
  ...
  const summary = [
    zh ? `${flat.length} 个代理` : `${flat.length} ${flat.length === 1 ? 'agent' : 'agents'}`,
    active > 0 ? (zh ? `${active} 个运行中` : `${active} active`) : '',
    failed > 0 ? (zh ? `${failed} 个失败` : `${failed} failed`) : '',
    tools > 0 ? (zh ? `${tools} 次工具调用` : `${tools} tools`) : '',
    files > 0 ? (zh ? `${files} 个文件` : `${files} files`) : '',
  ...
  return (
    <section className="grid min-w-0 gap-3">
      <p className="text-[0.66rem] font-medium uppercase tracking-wider text-muted-foreground/70">
        {zh ? group.label.replace('Delegation', '委派') : group.label} <span className="text-muted-foreground/50">·</span> {zh ? `${group.nodes.length} 个工作代理` : `${group.nodes.length} workers`}
        {activeWorkers > 0 ? <span className="text-primary/85"> · {zh ? `${activeWorkers} 个运行中` : `${activeWorkers} active`}</span> : null}
      </p>
...
function SubagentRow({ node, depth = 0, nowMs }: { node: SubagentNode; depth?: number; nowMs: number }) {
  ...
  const subtitle = [
    ...
    node.toolCount ? (zh ? `${node.toolCount} 次工具调用` : `${node.toolCount} tools`) : '',
    fmtTokens((node.inputTokens ?? 0) + (node.outputTokens ?? 0)),
    zh ? `更新于 ${fmtAge(node.updatedAt, nowMs, true)}` : `updated ${fmtAge(node.updatedAt, nowMs)}`
  ].filter(Boolean)
  ...
  return (
    <div className={cn('grid min-w-0 max-w-full gap-2', depth > 0 && 'pl-4')} data-slot="tool-block" ref={enterRef}>
      <button
        ...
        type="button"
      >
        <span className="mt-0.5 flex h-[1.1rem] shrink-0 items-center">{statusGlyph(node.status, zh)}</span>
      ...
      {open && fileLines.length > 0 ? (
        <div className="grid min-w-0 gap-0.5 pl-6">
          <p className="text-[0.58rem] font-medium tracking-wider text-muted-foreground/60 uppercase">{zh ? '文件' : 'Files'}</p>
          ...
          {fileLines.length > 8 ? (
            <p className="font-mono text-[0.67rem] leading-relaxed text-muted-foreground/65">
              {zh ? `另外还有 ${fileLines.length - 8} 个文件` : `+${fileLines.length - 8} more files`}
            </p>
          ) : null}
        </div>
      ) : null}
