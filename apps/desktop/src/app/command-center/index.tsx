...
export function CommandCenterView({
  ...
  return (
    <OverlayView
      closeLabel={zh ? '关闭命令中心' : 'Close command center'}
      headerContent={
        <OverlaySearchInput
          containerClassName="w-[min(36rem,calc(100vw-32rem))] min-w-80"
          loading={searchLoading}
          onChange={next => setQuery(next)}
          placeholder={zh ? '搜索会话、页面和操作' : 'Search sessions, views, and actions'}
          value={query}
        />
      }
      onClose={onClose}
        ...
        <OverlaySidebar>
          {SECTIONS.map(value => (
            <OverlayNavItem
              ...
              key={value}
              label={zh ? ({ sessions: '会话', system: '系统', usage: '用量' }[value]) : SECTION_LABELS[value]}
          ...
          <header className="mb-4 flex items-center justify-between gap-2">
            <div>
              <h2 className="text-sm font-semibold text-foreground">
                {zh ? ({ sessions: '会话', system: '系统', usage: '用量' }[section]) : SECTION_LABELS[section]}
              </h2>
              <p className="text-xs text-muted-foreground">
                {zh
                  ? ({ sessions: '搜索和管理会话', system: '状态、日志和系统操作', usage: '查看 token、费用和技能活动' }[section])
            ...
            {section === 'system' && (
              <OverlayActionButton disabled={systemLoading} onClick={() => void refreshSystem()}>
                ...
                {systemLoading ? (zh ? '刷新中…' : 'Refreshing...') : zh ? '刷新' : 'Refresh'}
              </OverlayActionButton>
            )}
            {section === 'usage' && (
              <OverlayActionButton disabled={usageLoading} onClick={() => void refreshUsage(usagePeriod)}>
                ...
                {usageLoading ? (zh ? '刷新中…' : 'Refreshing...') : zh ? '刷新' : 'Refresh'}
              </OverlayActionButton>
            )}
          </header>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pr-1">
              {!hasGlobalSearchResults ? (
                <OverlayCard className="px-3 py-4 text-sm text-muted-foreground">
                  {zh ? '没有找到匹配结果。' : 'No matching results found.'}
                </OverlayCard>
              ) : (
                            ...
                            <OverlayCard className="p-2.5" key={`${group.id}:${result.sessionId}:${result.snippet}`}>
                              ...
                              <div className="mt-2 flex gap-1">
                                <OverlayIconButton
                                  ...
                                  title={pinned ? (zh ? '取消置顶会话' : 'Unpin session') : zh ? '置顶会话' : 'Pin session'}
                                >
                                  {pinned ? (
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-foreground">
                            {status.gateway_running ? (zh ? '消息网关运行中' : 'Messaging gateway running') : zh ? '消息网关已停止' : 'Messaging gateway stopped'}
                          </span>
                        </div>
                        <div className="mt-1 text-xs text-muted-foreground">
                          {zh ? `Hermes ${status.version} · 活跃会话 ${status.active_sessions}` : `Hermes ${status.version} · Active sessions ${status.active_sessions}`}
                        </div>
                      </div>
                      <div className="flex shrink-0 items-center gap-1.5 whitespace-nowrap">
                        <OverlayActionButton className="h-7 px-2.5" onClick={() => void runSystemAction('restart')}>
                          {zh ? '重启消息网关' : 'Restart messaging'}
                        </OverlayActionButton>
                        <OverlayActionButton className="h-7 px-2.5" onClick={() => void runSystemAction('update')}>
                          {zh ? '更新 Hermes' : 'Update Hermes'}
                        </OverlayActionButton>
                      </div>
                    </div>
                    {systemAction && (
                      <div className="text-xs text-muted-foreground">
                        {systemAction.name} ·{' '}
                        {systemAction.running ? (zh ? '运行中' : 'running') : systemAction.exit_code === 0 ? (zh ? '已完成' : 'done') : zh ? '失败' : 'failed'}
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="text-xs text-muted-foreground">{zh ? '正在加载状态…' : 'Loading status...'}</div>
                )}
              </OverlayCard>

              <OverlayCard className="min-h-0 overflow-hidden p-2">
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs font-medium text-muted-foreground">{zh ? '最近日志' : 'Recent logs'}</span>
                  {systemError && (
                ...
                <pre className="h-full min-h-0 overflow-auto whitespace-pre-wrap wrap-break-word font-mono text-[0.65rem] leading-relaxed text-muted-foreground">
                  {logs.length ? logs.join('\n') : zh ? '还没有加载到日志。' : 'No logs loaded yet.'}
                </pre>
              </OverlayCard>
            </div>
          )}
      ...
      <OverlayCard className="p-3">
        {totals ? (
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <UsageStat label={zh ? '会话数' : 'Sessions'} value={formatInteger(totals.total_sessions)} />
            <UsageStat label={zh ? 'API 调用' : 'API calls'} value={formatInteger(totals.total_api_calls)} />
            <UsageStat
              label={zh ? '输入/输出 Token' : 'Tokens in/out'}
          ...
          <div className="text-xs text-muted-foreground">{zh ? '正在加载用量…' : 'Loading usage...'}</div>
        ) : (
          <div className="text-xs text-muted-foreground">
            {zh ? `最近 ${period} 天还没有用量记录。` : `No usage in the last ${period} days.`}{' '}
      ...
      <div className="grid min-h-0 grid-rows-[auto_minmax(0,1fr)] gap-3">
        <OverlayCard className="p-3">
          <div className="mb-2 flex items-baseline justify-between">
            <span className="text-xs font-medium text-muted-foreground">{zh ? '每日 Token' : 'Daily tokens'}</span>
            <span className="flex items-center gap-3 text-[0.65rem] text-muted-foreground">
              <span className="inline-flex items-center gap-1">
                <span className="size-2 bg-[color:var(--dt-primary)]/60" /> {zh ? '输入' : 'input'}
              </span>
              <span className="inline-flex items-center gap-1">
                <span className="size-2 bg-emerald-500/70" /> {zh ? '输出' : 'output'}
              </span>
            </span>
          </div>
          {daily.length === 0 ? (
            <div className="grid h-24 place-items-center text-xs text-muted-foreground">{zh ? '这段时间还没有每日活动。' : 'No daily activity.'}</div>
          ) : (
            <>
        ...
        <OverlayCard className="min-h-0 overflow-auto p-2">
          <div className="grid gap-3 sm:grid-cols-2">
            <section className="min-w-0">
              ...
              {byModel.length === 0 ? (
                <div className="text-xs text-muted-foreground">{zh ? '还没有模型使用记录。' : 'No model usage yet.'}</div>
              ) : (
            ...
            <section className="min-w-0">
              ...
              {topSkills.length === 0 ? (
                <div className="text-xs text-muted-foreground">{zh ? '还没有技能活动记录。' : 'No skill activity yet.'}</div>
              ) : (
                <ul className="space-y-1">
                  {topSkills.slice(0, 6).map(entry => (
                    ...
                    >
                      ...
                      <span className="shrink-0 text-[0.65rem] text-muted-foreground">
                        {zh ? `${entry.total_count.toLocaleString()} 次动作` : `${entry.total_count.toLocaleString()} actions`}
                      </span>
                    </li>
                  ))}
                </ul>
