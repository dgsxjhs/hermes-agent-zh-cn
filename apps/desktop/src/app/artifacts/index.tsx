...
export function ArtifactsView({ setStatusbarItemGroup: _setStatusbarItemGroup, ...props }: ArtifactsViewProps) {
  ...
  return (
      ...
      filters={
        <>
          <TextTab active={kindFilter === 'all'} onClick={() => setKindFilter('all')}>
            {zh ? '全部' : 'All'} <TextTabMeta>({counts.all})</TextTabMeta>
          </TextTab>
          <TextTab active={kindFilter === 'image'} onClick={() => setKindFilter('image')}>
            {zh ? '图片' : 'Images'} <TextTabMeta>({counts.image})</TextTabMeta>
          </TextTab>
          <TextTab active={kindFilter === 'file'} onClick={() => setKindFilter('file')}>
            {zh ? '文件' : 'Files'} <TextTabMeta>({counts.file})</TextTabMeta>
          </TextTab>
          <TextTab active={kindFilter === 'link'} onClick={() => setKindFilter('link')}>
            {zh ? '链接' : 'Links'} <TextTabMeta>({counts.link})</TextTabMeta>
          </TextTab>
        </>
      }
      onSearchChange={setQuery}
      searchPlaceholder={zh ? '搜索产物…' : 'Search artifacts...'}
      searchTrailingAction={
        <Button
          aria-label={refreshing ? (zh ? '正在刷新产物' : 'Refreshing artifacts') : zh ? '刷新产物' : 'Refresh artifacts'}
          className="text-(--ui-text-tertiary) hover:bg-transparent hover:text-foreground"
          disabled={refreshing}
          onClick={() => void refreshArtifacts()}
          size="icon-xs"
          title={refreshing ? (zh ? '正在刷新产物' : 'Refreshing artifacts') : zh ? '刷新产物' : 'Refresh artifacts'}
          type="button"
          variant="ghost"
        >
    ...
    >
      {!artifacts ? (
        <PageLoader label={zh ? '正在整理最近会话产物' : 'Indexing recent session artifacts'} />
      ) : visibleArtifacts.length === 0 ? (
        <div className="grid h-full place-items-center px-6 text-center">
          <div>
            <div className="text-sm font-medium">{zh ? '还没有找到产物' : 'No artifacts found'}</div>
            <div className="mt-1 text-xs text-muted-foreground">
              {zh ? '会话里生成的图片和文件输出，之后都会显示在这里。' : 'Generated images and file outputs will appear here as sessions produce them.'}
            </div>
          </div>
        </div>
      ) : (
            ...
            {paginationItems(page, pageCount).map((item, index) => (
              <PaginationItem key={`${item}-${index}`}>
                {item === 'ellipsis' ? (
                  ...
                  <PaginationButton
                    aria-label={zh ? `转到第 ${item} 页` : `Go to ${itemLabel} page ${item}`}
                    isActive={page === item}
